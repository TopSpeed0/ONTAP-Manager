...
if ($SetupVault) {
    ...
    # Resolve ONTAP username from credential registry → S3_Config → fallback 'admin'
    $getCredScript = Join-Path $workspaceRoot "scripts\credentials\Get-Credential.ps1"
    $ontapUsername = $s3Cfg.OntapUsername
    if ([string]::IsNullOrWhiteSpace($ontapUsername)) {
        $registryPath = Join-Path $workspaceRoot 'credentials\credentials.json'
        if (Test-Path -LiteralPath $registryPath) {
            $registry = Get-Content -LiteralPath $registryPath -Raw | ConvertFrom-Json
            if ($registry.PSObject.Properties[$credName]) {
                $ontapUsername = $registry.$credName.UserName
            }
        }
    }
    if ([string]::IsNullOrWhiteSpace($ontapUsername)) { $ontapUsername = 'admin' }
    ...
    Write-Host "  ONTAP credential: $credName.cred (user: $ontapUsername)" -ForegroundColor White

...
if ([string]::IsNullOrWhiteSpace($ClusterName)) {
    ...
    $clEntry = $ONTAP_Clusters | Where-Object { $_.cluster -eq $ClusterName -or $_.Alias -eq $ClusterName }
}
if (-not $clEntry) {
    $available = ($ONTAP_Clusters | ForEach-Object { $_.cluster }) -join ', '
    throw "Unknown cluster '$ClusterName'. Available: $available"
}
$Cluster = if ($clEntry.FallbackIP) { $clEntry.FallbackIP } else { $clEntry.cluster }

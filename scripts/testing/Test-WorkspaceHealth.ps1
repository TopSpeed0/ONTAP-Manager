...
.DESCRIPTION
    ...
    - Skill file integrity (all tracked project-local SKILL.md files present)
...
.EXAMPLE
    .\Test-WorkspaceHealth.ps1 -Cluster <cluster> -AggWarnPct 75
...
$scriptsToValidate = @(
    ...
    @{ Name = 'scripts\snapshots\Get-SnapshotComparison.ps1'; Path = "$rootDir\scripts\snapshots\Get-SnapshotComparison.ps1" }
    @{ Name = 'scripts\dfs-cleanup\Invoke-DFSDecommission.ps1'; Path = "$rootDir\scripts\dfs-cleanup\Invoke-DFSDecommission.ps1" }
    @{ Name = 'scripts\dfs-cleanup\Get-DFSCleanupAnalytics.psm1'; Path = "$rootDir\scripts\dfs-cleanup\Get-DFSCleanupAnalytics.psm1" }
...
if ($configLoaded) {
    ...
    foreach ($cl in $global:ONTAP_Clusters) {
        $missing = @()
        if (-not $cl.cluster)      { $missing += 'cluster' }
        if (-not $cl.CsvPrefix)    { $missing += 'CsvPrefix' }
        if ($missing.Count -gt 0) {
            Add-TestResult -Category 'Config' -TestName "Cluster: $($cl.cluster ?? '(unknown)')" -Status 'Fail' -Message "Missing: $($missing -join ', ')"
        } else {
            Add-TestResult -Category 'Config' -TestName "Cluster: $($cl.cluster)" -Status 'Pass' -Message "Connect=$($cl.cluster) CSV=$($cl.CsvPrefix) IP=$($cl.FallbackIP ?? 'DNS')"
        }
    }
    ...
    foreach ($cl in $global:ONTAP_Clusters) {
        ...
        # Connect function
        if (Get-Command $cluster -ErrorAction SilentlyContinue) {
            Add-TestResult -Category 'Functions' -TestName "$cluster (connect)" -Status 'Pass'
        } else {
            Add-TestResult -Category 'Functions' -TestName "$cluster (connect)" -Status 'Fail' -Message 'Not found'
        }
        ...
        # Alias (if different from cluster)
...
if ($SkipCredentials) {
    ...
    $registryPath = Join-Path $credDir 'credentials.json'
    if (Test-Path -LiteralPath $registryPath) {
        try {
            $registry = Get-Content -LiteralPath $registryPath -Raw | ConvertFrom-Json
            $regCount = @($registry.PSObject.Properties).Count
            $unmapped = @($credFiles | Where-Object { -not $registry.PSObject.Properties[$_.BaseName] })
            if ($unmapped.Count -eq 0) {
                Add-TestResult -Category 'Credentials' -TestName 'credentials.json' -Status 'Pass' -Message "$regCount entries — all .cred files have username mappings"
            } else {
                Add-TestResult -Category 'Credentials' -TestName 'credentials.json' -Status 'Warn' -Message "$regCount entries, $($unmapped.Count) unmapped: $($unmapped.BaseName -join ', ')"
            }
        } catch {
            Add-TestResult -Category 'Credentials' -TestName 'credentials.json' -Status 'Fail' -Message "Invalid JSON: $($_.Exception.Message)"
        }
    } else {
        Add-TestResult -Category 'Credentials' -TestName 'credentials.json' -Status 'Warn' -Message 'Registry file missing — run New-Credential.ps1 -Name <name> -UserName <user> to create'
    }
}
...
if ($SkipConnectivity) {
    ...
    foreach ($cl in $global:ONTAP_Clusters) {
        $host_ = if ($cl.FallbackIP) { $cl.FallbackIP } else { $cl.cluster }
        $label = "$($cl.cluster) ($host_)"

        # DNS resolution (skip for IPs)
        if ($host_ -notmatch '^\d{1,3}(\.\d{1,3}){3}$') {
            try {
                $dns = [System.Net.Dns]::GetHostAddresses($host_)
                Add-TestResult -Category 'Network' -TestName "DNS: $($cl.cluster)" -Status 'Pass' -Message "$host_ -> $($dns[0])"
            } catch {
                Add-TestResult -Category 'Network' -TestName "DNS: $($cl.cluster)" -Status 'Fail' -Message "Cannot resolve $host_"
            }
        } else {
            Add-TestResult -Category 'Network' -TestName "DNS: $($cl.cluster)" -Status 'Pass' -Message "IP address (no DNS needed): $host_"
        }
        ...
        $sw.Stop()
        if ($tcp22) {
            Add-TestResult -Category 'Network' -TestName "TCP/22: $($cl.cluster)" -Status 'Pass' -Message "${sw.ElapsedMilliseconds}ms" -DurationMs $sw.ElapsedMilliseconds
        } else {
            Add-TestResult -Category 'Network' -TestName "TCP/22: $($cl.cluster)" -Status 'Fail' -Message 'Port 22 unreachable' -DurationMs $sw.ElapsedMilliseconds
        }
        ...
        $sw.Stop()
        if ($tcp443) {
            Add-TestResult -Category 'Network' -TestName "TCP/443: $($cl.cluster)" -Status 'Pass' -Message "${sw.ElapsedMilliseconds}ms" -DurationMs $sw.ElapsedMilliseconds
        } else {
            Add-TestResult -Category 'Network' -TestName "TCP/443: $($cl.cluster)" -Status 'Warn' -Message 'Port 443 unreachable (REST API unavailable)' -DurationMs $sw.ElapsedMilliseconds
        }
    }
    ...
    foreach ($cl in $global:ONTAP_Clusters) {
        ...
        try {
            ...
            if ($ec -eq 0) {
                ...
                Add-TestResult -Category 'SSH' -TestName $cl.cluster -Status 'Pass' -Message $ver.Trim() -DurationMs $sw.ElapsedMilliseconds
            } elseif ($ec -eq 124) {
                Add-TestResult -Category 'SSH' -TestName $cl.cluster -Status 'Fail' -Message "SSH timed out after 15s" -DurationMs $sw.ElapsedMilliseconds
            } else {
                $errLine = [string]($sshOut | Select-Object -First 1) -replace "`r|`n", ''
                Add-TestResult -Category 'SSH' -TestName $cl.cluster -Status 'Fail' -Message "Exit $ec : $errLine" -DurationMs $sw.ElapsedMilliseconds
            }
        } catch {
            $sw.Stop()
            Add-TestResult -Category 'SSH' -TestName $cl.cluster -Status 'Fail' -Message $_.Exception.Message -DurationMs $sw.ElapsedMilliseconds
        }
    }
    ...
    if ($zapiCred -and $mod) {
        foreach ($cl in $global:ONTAP_Clusters) {
            $addr = if ($cl.FallbackIP) { $cl.FallbackIP } else { $cl.cluster }
            $sw = [System.Diagnostics.Stopwatch]::StartNew()
            try {
                $conn = Connect-NcController $addr -Credential $zapiCred -ErrorAction Stop
                $sw.Stop()
                Add-TestResult -Category 'ZAPI' -TestName $cl.cluster -Status 'Pass' -Message "v$($conn.Version) — $($conn.Name)" -DurationMs $sw.ElapsedMilliseconds
            } catch {
                $sw.Stop()
                Add-TestResult -Category 'ZAPI' -TestName $cl.cluster -Status 'Fail' -Message $_.Exception.Message -DurationMs $sw.ElapsedMilliseconds
            }
        }
    } elseif (-not $mod) {
    ...
    if (Get-Command 'wsl' -ErrorAction SilentlyContinue) {
        ...
        if ($configLoaded -and $global:Config.S3_Config -and $global:Config.S3_Config.Clusters) {
            ...
            foreach ($s3Prop in $global:Config.S3_Config.Clusters.PSObject.Properties) {
                ...
                $clObj = $global:ONTAP_Clusters | Where-Object { $_.cluster -eq $s3Name } | Select-Object -First 1
...
if ($configLoaded -and $global:Config.NDMP_Config) {
    ...
    foreach ($cl in $global:Config.ONTAP_Clusters) {
        if ($cl.NdmpPassword) {
            Add-TestResult -Category 'NDMP' -TestName "NdmpPassword: $($cl.cluster)" -Status 'Pass' -Message 'Set (value not logged)'
        } else {
            Add-TestResult -Category 'NDMP' -TestName "NdmpPassword: $($cl.cluster)" -Status 'Warn' -Message 'Not set — ndmpcopy to/from this cluster will fail'
        }
    }
} else {
...
if (-not $SkipConnectivity -and $configLoaded) {
    ...
    if ($csvTestCluster) {
        $csvSshFn = "$($csvTestCluster.cluster)-ssh"
...
if (-not $SkipConnectivity -and $configLoaded) {
    ...
    foreach ($restCl in $restClusters) {
        $restHost = if ($restCl.FallbackIP) { $restCl.FallbackIP } else { $restCl.cluster }
        $getCredScript = Join-Path $rootDir 'scripts\credentials\Get-Credential.ps1'
        ...
        try {
            ...
            $sw.Stop()
            Add-TestResult -Category 'REST-API' -TestName "/api/cluster ($($restCl.cluster))" -Status 'Pass' -Message "ONTAP $($restResp.version.full) — $($restResp.name)" -DurationMs $sw.ElapsedMilliseconds
        } catch {
            $sw.Stop()
            $restErr = $_.Exception.Message -replace "`r|`n",' '
            if ($restErr -match '401|403|Unauthorized') {
                Add-TestResult -Category 'REST-API' -TestName "/api/cluster ($($restCl.cluster))" -Status 'Warn' -Message "Endpoint reachable but auth failed" -DurationMs $sw.ElapsedMilliseconds
            } elseif ($restErr -match 'Credential file not found') {
                Add-TestResult -Category 'REST-API' -TestName "/api/cluster ($($restCl.cluster))" -Status 'Warn' -Message "Missing cred file: $($restCl.API_Cred).cred" -DurationMs $sw.ElapsedMilliseconds
            } else {
                Add-TestResult -Category 'REST-API' -TestName "/api/cluster ($($restCl.cluster))" -Status 'Fail' -Message $restErr -DurationMs $sw.ElapsedMilliseconds
            }
        }
    }
} elseif ($SkipConnectivity) {
        ...
        if ($template.ONTAP_Clusters -and $actual.ONTAP_Clusters) {
            ...
            foreach ($cl in $actual.ONTAP_Clusters) {
                ...
                if ($clMissing.Count -gt 0) {
                    Add-TestResult -Category 'TemplateDrift' -TestName "Cluster $($cl.cluster)" `
...
if ($SkipConnectivity) {
    ...
    $targetClusters = if ($Cluster) {
        $global:ONTAP_Clusters | Where-Object {
            $_.cluster -eq $Cluster -or $_.Alias -eq $Cluster
        }
    ...
    foreach ($cl in $targetClusters) {
        ...
        if ($configLoaded -and $global:Config.S3_Config -and $global:Config.S3_Config.Clusters) {
            $s3Entry = $global:Config.S3_Config.Clusters.PSObject.Properties | Where-Object {
                $_.Name -eq $cl.cluster -or $_.Name -eq $cl.Alias
            }
            if ($s3Entry) {

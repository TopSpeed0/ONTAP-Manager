...
Write-Host "Clusters in scope : $($targetClusters.cluster -join ', ')" -ForegroundColor Cyan
...
foreach ($cl in $targetClusters) {
    ...
    Write-Host "  $($cl.cluster) ($addr)..." -NoNewline
    try {
        $controllers[$cl.cluster] = Connect-NcController $addr
...
foreach ($clName in $controllers.Keys) {
    ...
    $clGroup = ($targetClusters | Where-Object { $_.cluster -eq $clName }).SnapmirrorGroup

...
$clEntry = $ONTAP_Clusters | Where-Object { $_.cluster -eq $ClusterName }
if (-not $clEntry) {
    $available = ($ONTAP_Clusters | ForEach-Object { $_.cluster }) -join ', '
    throw "Unknown cluster '$ClusterName'. Available: $available"
}
$sshHost = if ($clEntry.FallbackIP) { $clEntry.FallbackIP } else { $clEntry.cluster }
...
function Format-SmReport {
    ...
    $report = "📊 **SnapMirror Status — $($clEntry.cluster)** ($timestamp)`n`n"

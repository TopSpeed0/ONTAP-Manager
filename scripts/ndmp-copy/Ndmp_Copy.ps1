...
.DESCRIPTION
    ...
    Use -SrcCluster / -DstCluster with any Alias or cluster name from config.
.PARAMETER SrcCluster
    Source cluster Alias or cluster name (from config.json).
.PARAMETER DstCluster
    Destination cluster Alias or cluster name.
...
function Find-ClusterConfig {
    ...
    $match = $Config.ONTAP_Clusters | Where-Object {
        $_.Alias -eq $Name -or $_.cluster -eq $Name
    }
...
$SrcClusterConnect = $srcCfg.cluster
$DstClusterConnect = $dstCfg.cluster

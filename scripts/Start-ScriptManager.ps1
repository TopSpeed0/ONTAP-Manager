...
$scripts = @(
    ...
    [pscustomobject]@{
        Name        = 'Share Migration — New Config (Wizard)'
        Description = 'Interactive builder for Config_shareMig.json with automated AD/NetApp discovery'
        Script      = 'scripts\share-migration\New-ShareMigConfig.ps1'
        DefaultArgs = ''
        Category    = 'CIFS / Shares'
    }
    [pscustomobject]@{
        Name        = 'Share Migration — Manager (GUI)'
        Description = 'WPF GUI console for editing config and running migration modes'
        Script      = 'scripts\share-migration\Start-ShareMigManager.ps1'
        DefaultArgs = ''
        Category    = 'CIFS / Shares'
    }
    [pscustomobject]@{
        Name        = 'Share Migration — Debug All Modes'
        Description = 'Run all Share Migration modes with transcript capture to debug/'
        Script      = 'scripts\testing\Test-ShareMigScripts.ps1'
        DefaultArgs = ''
        Category    = 'Testing'
    }
    [pscustomobject]@{
    ...
    [pscustomobject]@{
        Name        = 'DFS Cleanup — Preflight (read-only)'
        Description = 'Check File System Analytics, scan completion, and access-time evidence before any DFS decision'
        Script      = 'scripts\dfs-cleanup\Invoke-DFSDecommission.ps1'
        DefaultArgs = '-Mode Preflight'
        Category    = 'DFS Cleanup'
    }
    [pscustomobject]@{
        Name        = 'DFS Cleanup — Offline Safety Tests'
        Description = 'Run offline parser, verdict, approval-gate, and deletion-safety checks; no cluster contact'
        Script      = 'scripts\testing\Test-DFSCleanupScripts.ps1'
        DefaultArgs = ''
        Category    = 'Testing'
    }
    [pscustomobject]@{
        Name        = 'Snapshot Comparison (read-only)'
        Description = 'Inventory snapshots on every configured cluster; report oldest, largest, and old/large candidates. Never deletes.'
        Script      = 'scripts\snapshots\Get-SnapshotComparison.ps1'
        DefaultArgs = ''
        Category    = 'Snapshots'
    }
    [pscustomobject]@{
        Name        = 'Snapshot Comparison — Tests (offline)'
        Description = 'Offline test suite for the snapshot comparison workflow — no cluster contact'
        Script      = 'scripts\testing\Test-SnapshotComparisonScripts.ps1'
        DefaultArgs = ''
        Category    = 'Testing'
    }
)

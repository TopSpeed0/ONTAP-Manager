...
if (-not $currentController) {
    ...
    $addr = if ($mainCluster.FallbackIP) { $mainCluster.FallbackIP } else { $mainCluster.cluster }
...
foreach ( $Quota in $NcQuota) {
    # $NcQuotaReport += Get-NcQuota -Qtree <qtree-name> -Volume <volume-name> -Target "$($Quota.Volume)/$($Quota.Qtree)"

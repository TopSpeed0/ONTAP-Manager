...
```powershell
# Preferred — native NetApp.ONTAP cmdlet (pipe to Select-Object to pick fields)
Get-NcNetInterface
# Fallback — only if Get-NcNetInterface is unavailable
...
Get-NcNetPort
# Fallback — only if Get-NcNetPort is unavailable
...
# Fallback — only if Get-NcNetPortBroadcastDomain is unavailable
...
# Preferred — native NetApp.ONTAP cmdlet
Get-NcNetRoute
# Fallback — only if Get-NcNetRoute is unavailable
...
# Fallback — only if Get-NcNetFailoverGroup is unavailable

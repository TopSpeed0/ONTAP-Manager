...
> **CRITICAL — Secret Key Capture**: The S3 secret key is shown **only once** during user creation or key regeneration. SSH non-interactive commands (`ssh admin@cluster "..."` or `-ssh -Command`) **swallow the secret key output**. Use the **ONTAP REST API** method below to reliably capture both access key and secret key.
...
#### Option A.2: Via curl (for Linux / DevOps pipelines)
```bash
SVM_UUID="<svm-uuid>"
BASE="https://<mgmt-lif>/api/protocols/s3/services/$SVM_UUID"

# Create S3 user (secret key shown ONLY in this response — save it!)
curl -k -u '<username>:<password>' \
  -X POST "$BASE/users" \
  -H "Content-Type: application/json" \
  -d '{"name": "my-service-user", "comment": "service X"}'

# Create policy scoped to specific buckets (bucket must exist!)
curl -k -u '<username>:<password>' \
  -X POST "$BASE/policies" \
  -H "Content-Type: application/json" \
  -d '{"name": "svc-x-policy", "statements": [{"effect": "allow", "actions": ["GetObject","PutObject","DeleteObject","ListBucket"], "resources": ["<existing-bucket>","<existing-bucket>/*"]}]}'

# Create group binding user to policy
curl -k -u '<username>:<password>' \
  -X POST "$BASE/groups" \
  -H "Content-Type: application/json" \
  -d '{"name": "svc-x-group", "users": [{"name": "my-service-user"}], "policies": [{"name": "svc-x-policy"}]}'

# List users
curl -k -u '<username>:<password>' "$BASE/users"

# Delete user
curl -k -u '<username>:<password>' -X DELETE "$BASE/users/my-service-user"

# Delete policy
curl -k -u '<username>:<password>' -X DELETE "$BASE/policies/svc-x-policy"

# Delete group (uses numeric ID, not name — get ID first)
GRP_ID=$(curl -sk -u '<username>:<password>' "$BASE/groups" | jq -r '.records[] | select(.name=="svc-x-group") | .id')
curl -k -u '<username>:<password>' -X DELETE "$BASE/groups/$GRP_ID"
```

> **REST API gotchas discovered**:
> - Policy `resources` must reference **existing bucket names** — ONTAP validates. Use `"*"` for all buckets.
> - Group DELETE uses numeric `id` (from GET), not the group name.
> - Secret key is returned **only** in the POST response — not retrievable later.
...
| REST API returns XML `AWS4-HMAC-SHA256` error on management LIF | S3 engine hijacks port 443 on the **entire broadcast domain** where an S3 data LIF exists. Custom service policies with only `management-https` are NOT enough — moving to a different port or node within the same broadcast domain also doesn't help. | The management LIF **must** use `default-management` service policy (which includes `data-core` and 11 services). Only `default-management` prevents the S3 engine from intercepting. |
...
#### S3 Engine Hijacks Port 443 on Entire Broadcast Domain
The S3 engine claims port 443 across the **entire broadcast domain** (VLAN) where any S3 data LIF exists — not just the specific LIF or physical port. A management LIF on the same broadcast domain will get intercepted regardless of its own service policy, unless it uses `default-management`.

**What doesn't work:**
- Custom service policy with only `management-https` + `management-dns-client`
- Moving the management LIF to a different physical port within the same broadcast domain
- Moving the management LIF to a different node within the same broadcast domain

**What works:**
- `default-management` service policy (includes `data-core` and 11 services total) — this prevents the S3 engine from intercepting
- Placing the management LIF on a different broadcast domain/VLAN entirely

**Fix:**
```
net int create -vserver <svm> -lif <mgmt_lif> -service-policy default-management \
  -address <ip> -netmask <mask> -home-node <node> -home-port <port>
```

#### Multi-Subnet REST API Access Pattern
When DevOps users on a different subnet (e.g., lab network) need REST API access to manage S3 users but can't reach the corporate management LIF:

1. Create a new LIF on their subnet with `default-management` — this is the only change needed
2. They can use their existing ONTAP management credentials (e.g., `svm_s3_dev` with `vsadmin` role)
3. No new users or custom roles required — the problem is network reachability, not permissions

For least-privilege (optional): create a restricted role scoped to S3 user/policy/group operations only:
```
security login role create -vserver <svm> -role s3-user-admin -cmddirname DEFAULT -access none
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server" -access readonly
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server user" -access all
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server policy" -access all
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server group" -access all
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server bucket policy" -access all
security login role create -vserver <svm> -role s3-user-admin -cmddirname "vserver object-store-server bucket" -access readonly
security login create -vserver <svm> -user-or-group-name <username> -application http -authentication-method password -role s3-user-admin
```

#### ONTAP Admin User vs S3 Object Store User — Two Separate Identity Systems
These are completely independent — different names, different auth, different purpose:

| | ONTAP Management User | S3 Object Store User |
|---|---|---|
| **Examples** | `admin`, `svm_s3_dev`, `s3_devops_api` | `sm_s3_dev`, `root`, `my-service-user` |
| **Auth** | username + password | access_key + secret_key |
| **Plane** | Management (REST API, SSH, System Manager) | Data (S3 protocol — aws-cli, rclone, SDKs) |
| **Created via** | `security login create` | `vserver object-store-server user create` |
| **Can create S3 users?** | Yes (via REST API or CLI) | **No** — S3 protocol has no user management |
| **Can access objects?** | No | Yes |

**Key insight**: Creating S3 users requires the ONTAP REST API (`POST /api/protocols/s3/services/{uuid}/users`) or CLI — there is no S3 protocol equivalent. Not on ONTAP, not on AWS (uses IAM), not on any S3-compatible platform.

The Ansible playbook uses ONTAP admin credentials and only writes the S3 username into the bucket policy. No S3 access keys are needed by the playbook.

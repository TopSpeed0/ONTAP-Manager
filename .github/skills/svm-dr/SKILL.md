...
argument-hint: 'Provide source and destination cluster names (e.g., <source-cluster> to <dest-cluster>)'
---
...
## Prerequisites
- Both clusters must be accessible via SSH (use `<cluster-ssh>`, `<cluster-ssh>`, or the appropriate alias)
...
Ask the user for:
1. **Source cluster** (e.g., `<cluster-name>`) and the **SVM name** to protect
2. **Destination cluster** (e.g., `<cluster-name>`)
3. **Destination aggregate(s)** to use (or discover automatically)
4. **Network details** — whether to replicate LIFs or create new ones at DR site

**Pre-flight checks — run ALL of these before proceeding:**
```powershell
# Check for stale vserver peers that may conflict
<cluster-ssh> -Command "vserver peer show -vserver <source-svm> -fields peer-vserver,peer-cluster,peer-state"

# Check for leftover S3/object-store configs (these BLOCK SVM-DR)
<cluster-ssh> -Command "vserver object-store-server show -vserver <source-svm>"
<cluster-ssh> -Command "vserver object-store-server bucket show -vserver <source-svm>"
<cluster-ssh> -Command "vserver object-store-server user show -vserver <source-svm>"
<cluster-ssh> -Command "vserver object-store-server policy show -vserver <source-svm>"

# Check for existing SnapMirror relationships on source SVM
<cluster-ssh> -Command "snapmirror show -source-path <source-svm>: -fields destination-path,state"

# Check if destination SVM already exists
<cluster-ssh> -Command "vserver show -vserver <dest-svm>"

# Check destination aggregate free space
<cluster-ssh> -Command "aggr show -fields aggregate,size,availsize,node"
```

**If S3 remnants are found, delete them (with user confirmation):**
```powershell
# Delete in order: buckets → users → policies → server
<cluster-ssh> -Command "vserver object-store-server bucket delete -vserver <svm> -bucket <name>"
<cluster-ssh> -Command "vserver object-store-server user delete -vserver <svm> -user <name>"
<cluster-ssh> -Command "vserver object-store-server policy delete -vserver <svm> -policy <name>"
<cluster-ssh> -Command "vserver object-store-server delete -vserver <svm>"
```

**If stale vserver peers conflict, delete them (with user confirmation):**
```powershell
<cluster-ssh> -Command "vserver peer delete -vserver <source-svm> -peer-vserver <stale-peer>"
```
...
<cluster-ssh> -Command "cluster peer show"
...
<cluster-ssh> -Command "net int create -vserver <source-cluster> -lif <ic-lif-name> -role intercluster -home-node <node> -home-port <port> -address <ip> -netmask <mask>"

# Destination: create intercluster LIFs (one per node)
<cluster-ssh> -Command "net int create -vserver <dest-cluster> -lif <ic-lif-name> -role intercluster -home-node <node> -home-port <port> -address <ip> -netmask <mask>"

# Source: create cluster peer (generates passphrase)
<cluster-ssh> -Command "cluster peer create -generate-passphrase -offer-expiration 2days -peer-addrs <dest-ic-lif-ips>"

# Destination: accept peering
<cluster-ssh> -Command "cluster peer create -peer-addrs <source-ic-lif-ips> -passphrase <passphrase>"
```

### Step 2 — Verify / Create Vserver Peering
```powershell
# Check existing vserver peers
<cluster-ssh> -Command "vserver peer show"

# Create vserver peer
<cluster-ssh> -Command "vserver peer create -vserver <source-svm> -peer-vserver <dest-svm> -peer-cluster <dest-cluster> -applications snapmirror"

# Accept on destination
<cluster-ssh> -Command "vserver peer accept -vserver <dest-svm> -peer-vserver <source-svm>"
```

#### Same-Name SVM Handling (IMPORTANT)
When source and destination SVMs have the **same name** (e.g., both `<svm-name>`):
...
<cluster-ssh> -Command "vserver peer create -vserver <svm> -peer-vserver <svm> -peer-cluster <dest-cluster> -applications snapmirror -local-name <svm>_dr"
```
...
<cluster-ssh> -Command "vserver peer modify-local-name -peer-cluster <source-cluster> -peer-vserver <svm> -new-name <svm>_med1"
```
- **The renamed local-name must be used in `snapmirror create -source-path`** on the destination cluster
- Example: if destination renamed peer to `<svm-peer-name>`, then:
```powershell
<cluster-ssh> -Command "snapmirror create -source-path <svm-peer-name>: -destination-path <svm-name>: ..."
```
...
```powershell
<cluster-ssh> -Command "vserver create -vserver <dest-svm> -subtype dp-destination"
```
Verify:
```powershell
<cluster-ssh> -Command "vserver show -vserver <dest-svm> -fields vserver,type,subtype,admin-state"
...
```powershell
<cluster-ssh> -Command "job schedule cron create -name svm_dr_sched -dayofweek * -hour 0 -minute 0"
```
...
<cluster-ssh> -Command "snapmirror create -source-path <source-svm-or-peer-localname>: -destination-path <dest-svm>: -type XDP -identity-preserve true"
...
```powershell
<cluster-ssh> -Command "snapmirror initialize -source-path <source-svm>: -destination-path <dest-svm>:"
```

### Step 6 — Validate
```powershell
# Check SnapMirror status
<cluster-ssh> -Command "snapmirror show -fields source-path,destination-path,state,status,healthy"

# Verify SVM on destination
<cluster-ssh> -Command "vserver show -vserver <dest-svm> -fields state,subtype,admin-state"

# Verify volumes replicated
<cluster-ssh> -Command "vol show -vserver <dest-svm> -fields volume,type,state"
```
...
<cluster-ssh> -Command "snapmirror quiesce -source-path <source-svm>: -destination-path <dest-svm>:"

# 2. Abort — stop any ongoing transfers
<cluster-ssh> -Command "snapmirror abort -source-path <source-svm>: -destination-path <dest-svm>:"

# 3. Break the SnapMirror relationship
<cluster-ssh> -Command "snapmirror break -source-path <source-svm>: -destination-path <dest-svm>:"

# 4. If identity-preserve true was used, stop the source SVM first
<cluster-ssh> -Command "vserver stop -vserver <source-svm>"

# 5. Start the destination SVM
<cluster-ssh> -Command "vserver start -vserver <dest-svm>"
```
...
<cluster-ssh> -Command "snapmirror create -source-path <dest-svm>: -destination-path <source-svm>:"

# 2. Reverse resync (from original source cluster)
<cluster-ssh> -Command "snapmirror resync -source-path <dest-svm>: -destination-path <source-svm>:"
# Optional for ONTAP 9.11.1+: add -quick-resync true for faster resync

# 3. Stop the destination SVM (currently serving data)
<cluster-ssh> -Command "vserver stop -vserver <dest-svm>"

# 4. Final update
<cluster-ssh> -Command "snapmirror update -source-path <dest-svm>: -destination-path <source-svm>:"

# 5. Quiesce reversed relationship
<cluster-ssh> -Command "snapmirror quiesce -source-path <dest-svm>: -destination-path <source-svm>:"

# 6. Break reversed relationship
<cluster-ssh> -Command "snapmirror break -source-path <dest-svm>: -destination-path <source-svm>:"

# 7. Start original source SVM
<cluster-ssh> -Command "vserver start -vserver <source-svm>"

# 8. Re-establish original protection direction
<cluster-ssh> -Command "snapmirror resync -source-path <source-svm>: -destination-path <dest-svm>:"
```
...
1. **S3 cleanup**: Source had leftover S3 user (`<s3-user>`) and policy (`BackupAccesss`) from a previous config — had to delete both before `snapmirror create` would work
2. **Stale vserver peer**: Had an old peer `<svm-name> ↔ <s3-svm>` that needed deletion
3. **Same-name handling**: Both clusters use `<svm-name>` — used `-local-name <svm-dr-name>` on source, then renamed peer to `<svm-peer-name>` on destination with `vserver peer modify-local-name`
4. **Source-path in snapmirror**: Used the destination's peer local-name: `snapmirror create -source-path <svm-peer-name>: -destination-path <svm-name>:`

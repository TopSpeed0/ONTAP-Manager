...
# Keep the served documentation wired to the tracked project-local sources. These are
# documentation-only checks: missing files warn before the hub starts, rather than silently
# hiding a newly added capability behind the generic README.
$documentationSources = @(
    ...
    'scripts\share-migration\README.md',
    'scripts\dfs-cleanup\README.md'
)
$missingDocumentation = @($documentationSources | Where-Object {
    -not (Test-Path -LiteralPath (Join-Path $root $_))
})
if ($missingDocumentation) {
    Write-Warning "Documentation sources missing: $($missingDocumentation -join ', ')"
} else {
    Write-Host "Documentation sources verified: $($documentationSources.Count)" -ForegroundColor DarkGray
}

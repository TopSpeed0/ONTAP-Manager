...
**Corporate-proxy note**: if the user is on a network with TLS interception, `npx` may fail to fetch the package over HTTPS. If that happens, change `command` to the absolute path of `node.exe` and prepend `--use-system-ca`:

...
if (-not $clusterObj) {
    $validNames = ($config.ONTAP_Clusters | ForEach-Object { "$($_.Alias) ($($_.cluster))" }) -join ', '
    throw "Cluster '$Cluster' not found in config.json. Valid: $validNames"
}

# Prefer FallbackIP (direct IP) over cluster hostname (DNS) for reliability
$ontapHostname = if ($clusterObj.FallbackIP) { $clusterObj.FallbackIP } else { $clusterObj.cluster }
...
if ($config.S3_Config -and $config.S3_Config.Clusters) {
    # Try Alias first, then cluster
    foreach ($key in @($clusterObj.Alias, $clusterObj.cluster)) {
...
# Resolve OntapUsername: S3_Config → credential registry → 'admin'
if (-not $OntapUsername) {
    $registryPath = Join-Path $workspaceRoot 'credentials\credentials.json'
    if (Test-Path -LiteralPath $registryPath) {
        $registry = Get-Content -LiteralPath $registryPath -Raw | ConvertFrom-Json
        if ($registry.PSObject.Properties[$CredentialName]) {
            $OntapUsername = $registry.$CredentialName.UserName
        }
    }
}
